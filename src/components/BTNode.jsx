import {
  ...
  HelpCircle, Layers, RefreshCcw, Zap, PlaySquare, Timer, ShieldAlert
} from 'lucide-react'
...
const ICON_BY_TYPE = {
  ...
  timeout: Timer,
  interrupt: ShieldAlert, // YENİ EKLENDİ
  action: Zap,
  ...
  return (
    <div
      className={[
        ...
        shapeClass,
        selected ? 'border-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.5)] z-10' : (isRoot ? '' : 'border-slate-600 shadow-md'),
        runState !== 'IDLE' ? runRing : '',
      ].join(' ')}
    >
      {!isRoot && (
        <Handle type="target" position={Position.Top} className="!h-3 !w-3 !bg-slate-200 !border-slate-800" />
      )}
      ...
      {canHaveChildren && (
        <Handle type="source" position={Position.Bottom} className="!h-3 !w-3 !bg-slate-200 !border-slate-800" />
      )}
    </div>
  )
}